...
n=ggplot(m,aes(ages,value,fill=variable,ymin=value-error,ymax=value+error))+geom_bar(stat="identity")+theme_classic(base_size = 20)+labs(x="Ages of Enrollment, years",y="Net Reclassification Index",fill="Metric Considered")+scale_fill_nejm()+ylim(c(-0.4,0.4))

ggsave(n,filename = "Figs/Supplemental/NRI_withPCS.tiff",width = 10,height = 6,dpi = 300)


n=ggplot(m,aes(ages,value,fill=variable))+geom_bar(stat="identity",position=position_dodge())+
theme_classic(base_size = 20)+labs(x="Ages of Enrollment (years)",y="Net Reclassification Index",fill="Metric Considered")+scale_fill_nejm()+ylim(c(-0.4,0.4))+
geom_errorbar(aes(ymin=value-error,ymax=value+error), width=.2,position=position_dodge(.9)) 

ggsave(n,filename = "Figs/Supplemental/NRI_withPCS.tiff",width = 10,height = 6,dpi = 300)
m[,c("value","error")]=round(m[,c(3:4)],3)
