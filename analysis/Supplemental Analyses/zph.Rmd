...
zph=readRDS("output/zph_amit_EQ.rds")
...
p <- ggplot(dat, aes(factor(variable), value)) +
  ...
  theme(plot.title = element_text(size = 20, hjust = 0.5),
        ...
        strip.text = element_text(size = 20, face = "bold"))+theme_classic()
...
colnames(ps)=c("Polygenic score","Pooled cohort equations","Low-density lipoprotein","Smoking","Systolic blood pressure","Diabetes mellitus","Male Sex","High-density lipoprotein","Total cholesterol")
...
bar=ggplot(mp,aes(x=variable,y=-log10(value),fill=variable))+geom_bar(stat = "identity")+theme_classic(base_size = 20)+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+labs(x="Risk Factor",fill="Risk Factor")+
scale_fill_manual(values=col_p, drop=TRUE,breaks=c("Diabetes mellitus","Male Sex","Smoking","Polygenic score","Low-density lipoprotein","Systolic blood pressure","Body Mass Index","Pooled cohort equations","Age","High-density lipoprotein","Total cholesterol")) 

ggsave(p,filename = "Figs/Supplemental/zph.tiff",dpi=300,create.dir = TRUE)
ggsave(bar,filename = "Figs/Supplemental/zph_pval.tiff",dpi=300,width = 8,height = 6)
```
