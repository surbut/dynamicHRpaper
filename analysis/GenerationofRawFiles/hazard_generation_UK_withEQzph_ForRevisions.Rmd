...
meltData=melt(zph_strat,id.vars = "age")
library(ggplot2)
p <- ggplot(meltData, aes(factor(variable), value)) 
p + geom_boxplot() + facet_wrap(~variable, scale="free")+labs(x="Variable",y="Pvalue(stratified analysis)")
...
saveRDS(zph_strat,"../../output/zph_amit_EQ_stratified.rds")
```
...
for(i in 1:bootstraps){
  #d=df[sample(nrow(df),10000),]
  d=df[sample(nrow(df),100000,replace = T),]
  # #    PRS fit
  sfit = coxph(Surv(phenos.CAD_censor_age, phenos.has_CAD) ~ prs_quant,
               data = d)
  #[i, 1] = exp(coef(sfit)[1])
...
library(ggplot2)
zph_unstratified=zph
m=melt(zph_unstratified)
p <- ggplot(m, aes(factor(variable), value)) 
p=p + geom_boxplot() + facet_wrap(~variable, scale="free")+labs(x="Variable",y="Pvalue(unstratified analysis)")
ggsave(plot = p,filename = "../../output/zph_unstratified.pdf",dpi = 300)



saveRDS(zph_unstratified,"../../output/zph_amit_EQ_unstratified.rds")
```
