...
# ### plot with PVE
# 
# f3=readRDS("~/Library/CloudStorage/Dropbox-Personal//fh_withPRS.rds")
# df=readRDS("~/Library/CloudStorage/Dropbox-Personal//fh-withoutprs.rds")
# 
# f3=readRDS("~/Dropbox/fh_prs.rds")
# df=readRDS("~/Dropbox/fh_full.rds")

f3=readRDS("data/fh_PRS.rds")
df=readRDS("data/fh_full.rds")
pcs=fread("~/Dropbox (Personal)/pca//genotype_data_nodup.eigenvec")
merged_df=merge(f3,pcs,by.x="shareid",by.y="#FID")

# Fit a linear model to residualize the PRS scores for the first 10 PC
model <- lm(V2 ~ PC1 + PC2 + PC3 + PC4 +
                  PC5 + PC6 + PC7 + PC8 +
                  PC9 + PC10, data = merged_df)

# Extract the residuals
merged_df$residuals <- residuals(model)

merged_df$scaled_score=scale(merged_df$residuals)
f3=merge(f3,merged_df[,c("shareid","scaled_score")],all.x = T)
f3$scaled_score[is.na(df$scaled_score)]=mean(merged_df$scaled_score)
saveRDS(f3,"output/fhs_prs_with_pc_corrrection.rds")
...
f3$prs.quant=scale(f3$scaled_score)
f3$sex=ifelse(f3$SEX==1,1,0) ## in framingham 2 is female, 1 is male

saveRDS(f3,"output/fhs_prs_with_pc_corrrection")
dat=data.frame(df %>%group_by(round(AGE1,0)) %>%summarise(length(AGE1)))
dat=dat[c(11:50),]
...
for(i in seq(1:nrow(dat))){
  ...
  d=d2=df[df$round_age==age|df$round_age==age-1|df$round_age==age+1,]
  f=f2=f3[f3$round_age==age|f3$round_age==age-1|f3$round_age==age+1,]
...
# saveRDS(lowerbound,"output/hazards_rate_lowerbound_fh_EQ_withPC_corrections.rds")
# saveRDS(upperbound,"output/hazards_rate_upperbound_fh_EQ_withPC_corrections.rds")
# # 
# saveRDS(hazards,"output/hazards_fh_EQ_withPC_corrections.rds") 

saveRDS(lowerbound,"output/hazards_rate_lowerbound_fh_EQ_withPC_corrections_2.rds")
saveRDS(upperbound,"output/hazards_rate_upperbound_fh_EQ_withPC_corrections_2.rds")
# 
saveRDS(hazards,"output/hazards_fh_EQ_withPC_corrections_2.rds") 
```
